...
async function ghRequest(env: Env, path: string, method = "GET", body?: unknown) {
...
async function ensureBranch(env: Env) {
  try {
    ...
    const main = await ghRequest(env, "git/ref/heads/main") as { object: { sha: string } };
...
export function buildTools(env: Env) {
  return {
    ...
    fetch_game_code: tool({
      description: "Fetch a range of lines from cosmos-infinite.html. Use to inspect existing code before editing.",
      ...
      execute: async ({ start_line, end_line }) => {
        ...
        return { lines: lines.join("\n"), start_line, total_fetched: lines.length };
      },
    }),

    /** Search cosmos-infinite.html for a pattern, returns matching lines */
    search_game_code: tool({
      description: "Search cosmos-infinite.html for a regex pattern. Returns matching line numbers and content.",
      inputSchema: z.object({
      ...
      execute: async ({ pattern, max_results }) => {
        ...
        for (let i = 0; i < lines.length && matches.length < max_results; i++) {
          if (rx.test(lines[i])) matches.push({ line: i + 1, text: lines[i].trim() });
        }
    ...
    commit_improvement: tool({
      description: "Apply an improvement to the game. Replaces old_string with new_string in the file and creates/updates a PR.",
      inputSchema: z.object({
        ...
        commit_message: z.string().describe("Descriptive commit message starting with feat:/fix:/ux:"),
        pr_title: z.string().describe("PR title — short, imperative"),
        feature_id: z.number().optional().describe("knowledge base feature ID this implements"),
      }),
      execute: async ({ old_string, new_string, commit_message, pr_title, feature_id }) => {
        await ensureBranch(env);

        // Get current file on agent branch
        const fileInfo = await ghRequest(env, `contents/${env.GAME_FILE_PATH}?ref=${env.AGENT_BRANCH}`) as {
          content: string; sha: string;
        };
        const content = atob(fileInfo.content.replace(/\n/g, ""));
        if (!content.includes(old_string)) {
          return { success: false, error: "old_string not found in file — code may have changed" };
        }
        ...
        try {
          ...
          const prs = await ghRequest(env, `pulls?head=${env.GITHUB_OWNER}:${env.AGENT_BRANCH}&state=open`) as Array<{ html_url: string }>;
        ...
        if (feature_id) {
          await env.DB.prepare("UPDATE features SET status='done', pr_url=?, updated_at=unixepoch() WHERE id=?")
            .bind(prUrl, feature_id).run();
        }
    ...
    query_knowledge: tool({
      description: "Query the knowledge base. Returns pending features sorted by priority, or recent research findings.",
      inputSchema: z.object({
        type: z.enum(["features", "research", "commits", "telemetry"]).default("features"),
        status: z.string().optional().describe("Filter features by status: pending, in_progress, done"),
    ...
    store_research: tool({
      description: "Store research findings from web lookups into the knowledge base for future cycles.",
      inputSchema: z.object({
        topic: z.string(),
        query: z.string(),
        findings: z.string().describe("Detailed findings, techniques, code snippets"),
        sources: z.array(z.string()).optional(),
      }),
      execute: async ({ topic, query, findings, sources }) => {
        await env.DB.prepare(
          "INSERT INTO research (topic, query, findings, sources) VALUES (?,?,?,?)"
        ).bind(topic, query, findings, JSON.stringify(sources ?? [])).run();
    ...
    fetch_url: tool({
      description: "Fetch a web page for research. Use for Three.js docs, game design patterns, A-Frame references.",
      inputSchema: z.object({
        url: z.string().describe("URL to fetch — must be a public documentation or reference page"),
        extract: z.string().describe("What specific information to extract from the page"),
      }),
      execute: async ({ url, extract }) => {
        const res = await fetch(url, { headers: { "User-Agent": "MerkabaGameAgent/1.0" } });
        if (!res.ok) return { error: `${res.status} fetching ${url}` };
        const text = await res.text();
        // Return first 8000 chars of relevant text content, strip tags
        const stripped = text.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").slice(0, 8000);
    ...
    claim_feature: tool({
      description: "Claim a feature to work on this cycle — marks it in_progress in the knowledge base.",
    ...
    add_to_backlog: tool({
      description: "Add a newly discovered bug or enhancement to the feature backlog.",
      inputSchema: z.object({
        title: z.string(),
      ...
      execute: async ({ title, description, phase, area, priority }) => {
        ...
        ).bind(title, description, phase, area, priority).run();
