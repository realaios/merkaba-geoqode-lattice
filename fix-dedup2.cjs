...
function verifyLine(idx, expected) {
  ...
  console.log(`  Line ${idx+1} [${pass ? 'OK' : 'FAIL'}]: "${actual.slice(0,70)}" (expected: "${expected}")`);
  return pass;
}
...
console.log(`  Removing rogue-planet: lines ${6287}–${6380} (${6380-6287+1} lines)`);
lines.splice(6286, 6380 - 6287 + 1);  // remove 94 lines (idx 6286, count 94)

// dark-matter-web: 0-indexed 5420-5508 inclusive
console.log(`  Removing dark-matter-web: lines ${5421}–${5509} (${5509-5421+1} lines)`);
...
const dmwCount = (joined.match(/AFRAME\.registerComponent\("dark-matter-web"/g) || []).length;
const rpCount  = (joined.match(/AFRAME\.registerComponent\("rogue-planet"/g) || []).length;
