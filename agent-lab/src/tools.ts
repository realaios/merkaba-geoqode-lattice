...
export function buildTools(env: Env) {
  return {
    fetch_lab_code: tool({
      description: "Fetch a range of lines from cosmos-lab.html. Use to inspect existing code before editing.",
    ...
    search_lab_code: tool({
      description: "Search cosmos-lab.html for a regex pattern. Returns matching line numbers and content.",
      inputSchema: z.object({
      ...
      execute: async ({ pattern, max_results }) => {
        ...
        return { matches, total_matches: matches.length, total_lines: lines.length };
      },
    }),

    commit_improvement: tool({
      description: "Apply an improvement to cosmos-lab.html. Replaces old_string with new_string and creates/updates a PR.",
      inputSchema: z.object({
        ...
        commit_message: z.string().describe("Descriptive commit message starting with feat:/fix:/ux:/experiment:"),
      ...
      execute: async ({ old_string, new_string, commit_message, pr_title, feature_id }) => {
        ...
        await env.DB.prepare(
          ...
          .bind(feature_id ?? null, prUrl, commit_message, new_string.split("\n").length, old_string.split("\n").length)
          .run();
    ...
    query_knowledge: tool({
      description: "Query the lab knowledge base for pending features, research, or commit history.",
      inputSchema: z.object({
        type: z.enum(["features", "research", "commits"]).default("features"),
        module: z.string().optional().describe("Filter by module: physics|chemistry|biology|engineering|hub|all"),
        status: z.string().optional().describe("Filter by status: pending|in_progress|done"),
      ...
      execute: async ({ type, module, status, limit }) => {
        if (type === "features") {
          ...
          if (module) { where += " AND (module = ? OR module = 'all')"; binds.push(module); }
          binds.push(limit);
    ...
    claim_feature: tool({
      description: "Claim a feature to work on this cycle — marks it in_progress.",
    ...
    store_research: tool({
      ...
      inputSchema: z.object({
        topic: z.string(),
        query: z.string(),
        findings: z.string().describe("Technical findings, Three.js/A-Frame code patterns, physics/chemistry formulas"),
    ...
    fetch_url: tool({
      description: "Fetch a web page for research — Three.js docs, physics simulations, A-Frame references.",
      inputSchema: z.object({
        url: z.string().describe("URL to fetch — public documentation or reference page"),
        extract: z.string().describe("What specific information to extract"),
      }),
      execute: async ({ url, extract }) => {
        const res = await fetch(url, { headers: { "User-Agent": "MerkabaLabAgent/1.0" } });
    ...
    add_to_backlog: tool({
      description: "Add a newly discovered experiment idea or bug to the lab feature backlog.",
      inputSchema: z.object({
        title: z.string(),
        description: z.string(),
        module: z.enum(["physics", "chemistry", "biology", "engineering", "hub", "all"]),
        area: z.enum(["experiment", "ux", "visual", "audio", "curriculum", "multiuser"]),
        phase: z.number().default(1),
        priority: z.number().min(1).max(10).default(5),
      }),
      execute: async ({ title, description, module, area, phase, priority }) => {
        const res = await env.DB.prepare(
          "INSERT INTO features (title, description, module, area, phase, priority) VALUES (?,?,?,?,?,?)",
        ).bind(title, description, module, area, phase, priority).run();
